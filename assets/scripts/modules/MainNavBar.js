...
class MainNavBar {
  ...
  onAddNewSubContentPanel() {
    ...
    $.initialize(".ui-tabs-sub", function () {
      ...
      if (
        self.enableOnAddNewNavItem &&
        $tabContent.parent().attr("id") == "app_tab_switcher"
      ) {
  ...
  update(removeSubPanelStyle) {
    ...
    this.mainNavItems.forEach(function (item) {
      if (
        item.$me.hasClass("ui-tabs-selected") &&
        item.$me.hasClass("ui-state-active")
      ) {
        item.active = true;
      } else {
        item.active = false;
...
class MainNavItem {
  ...
  constructor($navItem) {
    ...
    this.locked = false;
    if (
      this.$me.hasClass("ui-tabs-selected") &&
      this.$me.hasClass("ui-state-active")
    ) {
      this.active = true;
    }
    //events Click:
  ...
  onClick() {
    ...
    this.$contentLink.on("click", function () {
      ...
      if (
        self.$me.hasClass("ui-tabs-selected") &&
